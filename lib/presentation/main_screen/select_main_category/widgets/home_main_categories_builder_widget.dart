...
class HomeMainCategoriesBuilderWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return GridView.builder(
        shrinkWrap: true,
        padding:
            EdgeInsets.symmetric(vertical: AppSize.size(context).height * .015),
        ...
        itemBuilder: (context, index) => InkWell(
              onTap: () async {
                Navigator.pushNamed(
                    context, AppConstant.homeList[index].nextScreen!);
                // await context.setLocale(AppConstant.arabic);
              },
              child: Container(
                padding: EdgeInsets.symmetric(
                    vertical: AppSize.size(context).width * 0.02,
                    horizontal: AppSize.size(context).width * 0.05),
