...
class DateFromToColumnWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        dateRowBuilder(context: context,title: LocaleKeys.dateFrom.tr(),dateController: reportDateFormController,date: reportFrom),
        SizedBox(height: AppSize.size(context).height*0.015,),
        dateRowBuilder(context: context,title: LocaleKeys.dateTo.tr(),dateController: reportDateToController,date: reportTo),
        SizedBox(height: AppSize.size(context).height*0.015,),
  ...
  Container dateRowBuilder({BuildContext? context ,String ?title,TextEditingController ?dateController,
    DateTime ?date
  }) {
    return Container(
